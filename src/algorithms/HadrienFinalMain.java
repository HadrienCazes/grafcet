...
public class HadrienFinalMain extends Brain {


    public HadrienFinalMain() {
        super();
        gen = new Random();
    }
    ...
    private static final int MOVETOENEMY = 5;
    ...
    private static ArrayList<String> listStates = new ArrayList<String>(
            Arrays.asList("FACESOUTH", "MOVETASK", "MOVEBACK", "ANGLEDROIT", "FACEENEMY","MOVETOENEMY","SINK"));
    ...
    private void process(String message) {
        if (Integer.parseInt(message.split(":")[2]) == FIRE) {
            if (Integer.parseInt(message.split(":")[1]) == whoAmI){
                return;
            }
            targetX = Double.parseDouble(message.split(":")[3]);
            targetY = Double.parseDouble(message.split(":")[4]);
            faceEnemyAngle = computeAngle(targetX,targetY);
            //println(faceEnemyAngle);
            state = FACEENEMY;
        }
    }
    ...
    private double computeAngle(double x, double y) {
        if (myX<=x) return (Math.atan((y-myY)/(x-myX)));
        else return (Math.PI+Math.atan((y-myY)/(x-myX)));
    }
    ...
    public void step() {
        ...
        if (state == FACEENEMY){
            if (!isRoughlySameDirection(getHeading(), faceEnemyAngle)){
                stepTurn(teamDirection);
                return;
            }
            else{
