...
public class RandomFire extends Brain {
  ...
  public void step() {
    ...
    if (moveTask) {
       ...
       * endTaskDirection=(Math.random()-0.5)*Math.PI; turnRight=(endTaskDirection>0);
       * endTaskDirection+=getHeading(); if (turnRight)
       * stepTurn(Parameters.Direction.RIGHT); else
       * stepTurn(Parameters.Direction.LEFT); sendLogMessage("Turning point. Waza!");
       * }
       */
      if (endTaskCounter < 0) {
        turnTask = true;
