...
public class Hadrien2Main extends Brain {
  ...
  // FOR DEBUGGING
  private static ArrayList<String> listStates = new ArrayList<String>(Arrays.asList("TURNSOUTHTASK",
      "MOVETASK", "TURNLEFTTASK", "TOWARDENEMY", "MOVEBACKTASK", "SINK"));
  ...
  public void step() {
    ...
    if (debug && whoAmI == ALPHA && state != SINK) {
      sendLogMessage("MAIN BAS (" + (int) myX + ", " + (int) myY + ") state = " + printStateName());
    ...
    if (debug && whoAmI == BETA && state != SINK) {
      sendLogMessage(
          "MAIN MILIEU (" + (int) myX + ", " + (int) myY + ") state = " + printStateName());
      // sendLogMessage("#BETA *thinks* (x,y)= ("+(int)myX+", "+(int)myY+") theta=
      // "+(int)(myGetHeading()*180/(double)Math.PI)+"°. #State= "+state);
    }
    if (debug && whoAmI == GAMMA && state != SINK) {
      sendLogMessage(
          "MAIN HAUT (" + (int) myX + ", " + (int) myY + ") state = " + printStateName());
  ...
  private void firePosition(double x, double y) {
    if (myX <= x)
      fire(Math.atan2((y - myY), (double) (x - myX)));
    else
